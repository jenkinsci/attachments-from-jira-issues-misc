...
public class UrlChangeTrigger extends Trigger<BuildableItem> {
    ...
    public UrlChangeTrigger(String url, boolean useproxy) throws MalformedURLException {
        this(new URL(url), useproxy);
    }
    
    public UrlChangeTrigger(URL url, boolean useproxy) {
        this.url = url;
    ...
    public void run() {
        try {
            ...
            // check if usage of proxy is configured, and open URL via proxy, otherwise directly
            // (if no proxy is configured in Hudson, ProxyConfiguration opens the URL directly)
            URLConnection connection = useproxy? ProxyConfiguration.open( url ): url.openConnection();
            String currentMd5 = getMd5(connection.getInputStream());
    ...
    public static final class DescriptorImpl extends TriggerDescriptor {
        ...
        public UrlChangeTrigger newInstance(StaplerRequest req, JSONObject formData) throws FormException {
            ...
            boolean useproxy = formData.getBoolean("useproxy");
            try {
                return new UrlChangeTrigger(url,useproxy);
            } catch (MalformedURLException e) {
