...
// -------------------------- OTHER METHODS --------------------------
    ...
    private boolean accurevLogin(AccurevServer server, Map<String, String> accurevEnv, FilePath workspace,
        ...
        if (server != null && server.getUsername() != null && server.getUsername().length() > 0) {
            // Check if current user is already logged in to the correct server.
            ...
            StringOutputStream sos = new StringOutputStream();
            String resp = null;
            if (launcher.launch().cmds(cmd).envs(accurevEnv).stdout(sos).pwd(workspace).join() == 0) {
            	resp = sos.toString();
            	if (resp.contains("(not logged in)")) {
            		alreadyAuthenticated = false;
            	} else {
            		// Parse output of "accurev info" to extract current user and server.
            		String currentUser = "";
            		String currentServer = "";
            		final StringTokenizer st = new StringTokenizer(resp);
            		while (st.hasMoreTokens()) {
            			String s = st.nextToken();
            			if ("Principal:".equals(s) && st.hasMoreTokens()) {
            				currentUser = st.nextToken();
            			} else if ("Server".equals(s) && st.hasMoreTokens()) {
                                    s = st.nextToken();
                                    if ("name:".equals(s) && st.hasMoreTokens()) {
            				currentServer = st.nextToken();
                                    }
            			}
            		}
            		alreadyAuthenticated = server.getUsername().equals(currentUser) && server.getHost().equals(currentServer);
            	}
            } else {
            	alreadyAuthenticated = false;
            }
