...
public class PluginManagerInterceptor extends DefaultPluginManager {
    ...
    public void executeMojo(final MavenProject project, final MojoExecution mojoExecution, MavenSession session) throws ArtifactResolutionException, MojoExecutionException, MojoFailureException, ArtifactNotFoundException, InvalidDependencyVersionException, PluginManagerException, PluginConfigurationException {
        ...
        HudsonMojoExecutor mojoExecutor = null;
        if (isSurefireTest(mojoExecution.getMojoDescriptor())) {
            mojoExecutor = new HudsonSurefireMojoExecutor(config, project, mojoExecution, session);
        } else {
            mojoExecutor = new HudsonMojoExecutor(config, project, mojoExecution, session);
        }
        mojoExecutor.execute();
    }

    private boolean isSurefireTest(MojoDescriptor mojoDescriptor) {
        try{
            return mojoDescriptor.getPluginDescriptor().getGroupId().equals("org.apache.maven.plugins")
                && mojoDescriptor.getPluginDescriptor().getArtifactId().equals("maven-surefire-plugin")
                && mojoDescriptor.getGoal().equals("test");
        } catch (RuntimeException re) {
            return false;
        }
    }

    abstract class MojoConfig {
        PlexusConfiguration config;
        ExpressionEvaluator eval;
        abstract void callPost(Exception exception) throws IOException, InterruptedException;
    }

    class HudsonMojoExecutor {
        private MojoConfig config;
        private MavenProject project;
        private MojoExecution mojoExecution;
        private MavenSession session;

        HudsonMojoExecutor(MojoConfig config, MavenProject project, MojoExecution mojoExecution, MavenSession session){
            this.config = config;
            this.project = project;
            this.mojoExecution = mojoExecution;
            this.session = session;
        }
        void execute() throws ArtifactResolutionException, PluginManagerException, InvalidDependencyVersionException, ArtifactNotFoundException, PluginConfigurationException, MojoExecutionException, MojoFailureException {
            try {
                try {
                    // inside the executeMojo but before the mojo actually gets executed,
                    // we should be able to trap the mojo configuration.
                    PluginManagerInterceptor.super.executeMojo(project, mojoExecution, session);
                    config.callPost(null);
                } catch (MojoExecutionException e) {
                    config.callPost(e);
                    throw e;
                } catch (MojoFailureException e) {
                    handleMojoFailureException(config, e);
        ...
        void handleMojoFailureException(PluginManagerInterceptor.MojoConfig config, MojoFailureException e) throws IOException, InterruptedException, MojoFailureException {
            this.config.callPost(e);
            throw e;
        }
    }

    class HudsonSurefireMojoExecutor extends HudsonMojoExecutor {
        HudsonSurefireMojoExecutor(MojoConfig config, MavenProject project, MojoExecution mojoExecution, MavenSession session) {
            super(config, project, mojoExecution, session);
        }
